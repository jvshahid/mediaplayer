...
#ifdef LOGGING
#define NDK_INFO(...) __android_log_print(ANDROID_LOG_INFO, MODULE, __VA_ARGS__)
#else
#define NDK_INFO(...)
#endif
#define NDK_ERROR(...) __android_log_print(ANDROID_LOG_ERROR, MODULE, __VA_ARGS__)
...
{
  int i;
  for (i = 0; i < length - 1 && ((buffer[i] & 0xff) != 0xff || (buffer[i + 1] & 0xf0) != 0xf0); i++)
    ;
  return i;
}

static int
init_filters(JNIEnv *env,
             ...
             AVFilterContext **buffersrc_ctx)
{
  ...
  *filter_graph = avfilter_graph_alloc();
  ...
  if (ret < 0) {
    (*env)->ThrowNew(env, runtime_exception, "Cannot create audio buffer source");
    return ret;
  }
  /* buffer audio sink: to terminate the filter chain. */
  abuffersink_params = av_abuffersink_params_alloc();
  abuffersink_params->sample_fmts     = sample_fmts;
  ret = avfilter_graph_create_filter(buffersink_ctx, abuffersink, "out", NULL, abuffersink_params, *filter_graph);
  av_free(abuffersink_params);
  if (ret < 0) {
    (*env)->ThrowNew(env, runtime_exception, "Cannot create audio buffer sink");
    return ret;
  }
  ...
  if ((ret = avfilter_graph_parse(*filter_graph, filters_descr, &inputs, &outputs, NULL)) < 0) {
    (*env)->ThrowNew(env, runtime_exception, "cannot parse filter description");
    return ret;
  }
...
(JavaVM *vm, void *reserved)
{
  ...
  aac_info = (*env)->FindClass(env, "org/extremesolution/nilefm/media/AACInfo");
  aac_info = (*env)->NewGlobalRef(env, aac_info);
  decoding_info = (*env)->FindClass(env, "org/extremesolution/nilefm/media/DecodingInfo");
  decoding_info = (*env)->NewGlobalRef(env, decoding_info);
  aac_info_constr = (*env)->GetMethodID(env, aac_info, "<init>", "(II)V");
  decoding_info_constr = (*env)->GetMethodID(env, decoding_info, "<init>", "(I[S)V");
  jclass klazz = (*env)->FindClass(env, "org/extremesolution/nilefm/media/AACPlayer");
 ...
 * Class:     org_extremesolution_nilefm_media_AACPlayer
 * Method:    initNative
 * Signature: ([BI)Lorg/extremesolution/nilefm/media/AACInfo;
 */
JNIEXPORT jobject JNICALL
Java_org_extremesolution_nilefm_media_AACPlayer_initNative
(JNIEnv *env, jclass unused, jobject thiz, jbyteArray jbuffer, jint length)
{
  ...
  if (!(decoded_frame = avcodec_alloc_frame())) {
    (*env)->ThrowNew(env, runtime_exception, "Could not allocate audio frame");
    return NULL;
  }

  int rc = avcodec_decode_audio4(data->context, decoded_frame, &got_frame, &avpkt);
  if (rc < 0) {
  ...
  NDK_INFO("decoded first chunk, sample rate: %d, channels: %d, consumed: %d", data->context->sample_rate, data->context->channels, rc);

  rc = init_filters(env, filter_descr, &data->filter_graph, data->context, &data->buffersink_ctx, &data->buffersrc_ctx);
  if (rc < 0) {
    // init_filters will throw the appropriate exception
    // (*env)->ThrowNew(env, runtime_exception, "cannot initialize filters\n");
    avcodec_free_frame(&decoded_frame);
    return NULL;
  }
  ...
  return (*env)->NewObject(env, aac_info, aac_info_constr, data->context->sample_rate, data->context->channels);
}

/*
 * Class:     org_extremesolution_nilefm_media_AACPlayer
 * Method:    decodeNative
 * Signature: ([BI)Lorg/extremesolution/nilefm/media/DecodingInfo;
 */
JNIEXPORT jobject JNICALL
Java_org_extremesolution_nilefm_media_AACPlayer_decodeNative
(JNIEnv *env, jclass unused, jobject thiz, jbyteArray jbuffer, jint length)
{
  jobject result = NULL;
  ...
  internal_data_t *data = (internal_data_t *) (*env)->GetLongField(env, thiz, ptr_id);

  if (!data) {
    (*env)->ThrowNew(env, runtime_exception, "How is the data pointer NULL");
    return NULL;
  }
  ...
  while (avpkt.size > 1024) {
    ...
    if (offset > 0) {
      NDK_ERROR("Found sync at %d, %x, %x", offset, avpkt.data[0] & 0xff, avpkt.data[1] & 0xff);
      consumed += offset;
    }

    if (!decoded_frame) {
      if (!(decoded_frame = avcodec_alloc_frame())) {
        (*env)->ThrowNew(env, runtime_exception, "Cannot allocate frame");
        return NULL;
      }
    } else {
    ...
    int got_frame;
    int len = avcodec_decode_audio4(data->context, decoded_frame, &got_frame, &avpkt);
    if (len < 0) {
    ...
    if (got_frame) {
      ...
      if (av_buffersrc_add_frame(data->buffersrc_ctx, decoded_frame, 0) < 0) {
        (*env)->ThrowNew(env, runtime_exception, "Error while feeding the audio filtergraph");
        goto end;
      }
      ...
      while (1) {
        ...
        int ret = av_buffersink_get_buffer_ref(data->buffersink_ctx, &samplesref, 0);
        ...
        if (samplesref) {
          ...
          int size = p_end - p;
          pcm_samples = realloc(pcm_samples, sizeof(uint16_t) * (generated + size));
  ...
  result = (*env)->NewObject(env, decoding_info, decoding_info_constr, consumed, jpcm_samples);
...
Java_org_extremesolution_nilefm_media_AACPlayer_releaseNative
(JNIEnv *env, jclass unused, jobject thiz)
{
  internal_data_t *data = (internal_data_t *) (*env)->GetLongField(env, thiz, ptr_id);
  (*env)->SetLongField(env, thiz, ptr_id, NULL); /* set the pointer to NULL */
